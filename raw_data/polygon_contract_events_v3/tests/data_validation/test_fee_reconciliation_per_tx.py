...
def test_fee_reconciliation_per_tx(con):
    of_complete = complete_1m_ranges_for_paths(
        [
            "CTFExchange/order_filled",
            "NegRiskCtfExchange/order_filled",
        ]
    )
    fc_complete = complete_1m_ranges_for_paths(
        [
            "CTFExchange/fee_charged",
    ...
    for r in ranges:
        ...
        # Sample only the first and last 2 10K partitions per 1M range to
        # keep runtime bounded (100 10Ks/range × 6 ranges = too slow).
        all_10ks = sorted(ctf_of_10ks | nr_of_10ks)
        sampled_10ks = sorted(set(all_10ks[:2] + all_10ks[-2:]))

        for k in sampled_10ks:
            of_paths = []
            fc_paths = []
